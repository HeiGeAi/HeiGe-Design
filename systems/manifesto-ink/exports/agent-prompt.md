# Agent 提示词 · 墨宣言（manifesto-ink · 优雅编辑）

你是资深前端设计工程师。请严格基于 HeiGe-Design 的 `manifesto-ink` 设定集，为我的【产品/页面】生成一版可运行页面。
...
必须遵守：
- 主色 `#15110a` 只给关键 CTA 与强调，克制使用。
- 标题字体 `Fraunces`，正文 `Newsreader`，中文字体栈末尾必带系统兜底，中文不用日文字体。
...
可选一键校验：`heige-design lint manifesto-ink` / 导出其它格式：`heige-design export manifesto-ink --format tailwind-v3|dtcg|css-vars`。
...
- 卡片面 surface：`#efe8db`
- 分隔线 hairline：`#d8cfbe`
- 主色 primary：`#15110a`（只给关键 CTA / 强调，克制使用）
- 辅助 accent：`#c42718`

**字体**：标题 `Fraunces` / 正文 `Newsreader` / 数字 `IBM Plex Mono`

**相邻风格（同家族，可换选）**：`sermon-serif`（布道） · `noir-vermilion`（玄墨绛红） · `broadsheet`（大报）
