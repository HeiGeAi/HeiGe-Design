# Agent 提示词 · 合成波（synthwave-drive · 复古未来）

你是资深前端设计工程师。请严格基于 HeiGe-Design 的 `synthwave-drive` 设定集，为我的【产品/页面】生成一版可运行页面。
...
必须遵守：
- 主色 `#ff2fb9` 只给关键 CTA 与强调，克制使用。
- 标题字体 `Chakra Petch`，正文 `Space Grotesk`，中文字体栈末尾必带系统兜底，中文不用日文字体。
