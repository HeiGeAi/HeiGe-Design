# Agent 提示词 · 故宫红金（gugong-red · 东方新中式）

你是资深前端设计工程师。请严格基于 HeiGe-Design 的 `gugong-red` 设定集，为我的【产品/页面】生成一版可运行页面。
...
必须遵守：
- 主色 `#9A2B2B` 只给关键 CTA 与强调，克制使用。
- 标题字体 `Noto Serif SC`，正文 `Inter`，中文字体栈末尾必带系统兜底，中文不用日文字体。
...
可选一键校验：`heige-design lint gugong-red` / 导出其它格式：`heige-design export gugong-red --format tailwind-v3|dtcg|css-vars`。
...
- 卡片面 surface：`#FBF6EA`
- 分隔线 hairline：`#D9C7A0`
- 主色 primary：`#9A2B2B`（只给关键 CTA / 强调，克制使用）
- 辅助 gold：`#C9A227`
- 辅助 dai：`#2E3A4A`
- 辅助 seal：`#BE3626`

**字体**：标题 `Noto Serif SC` / 正文 `Inter` / 数字 `Playfair Display`

**相邻风格（同家族，可换选）**：`moxi-void`（水墨留白） · `song-celadon`（宋瓷极简） · `guochao-cyber`（赛博国潮）
